"""
puma_analytical_IK.py
Students should edit this scritp's function "puma_analytical_IK_calc" with their solution to Analytical IK for the puma.
"""
...
class GoalPoseSubscriber(Node):
    ...
    def __init__(self):
        super().__init__('puma_analytical_IK')
    ...
    def goal_pose_callback(self, pose_msg):
        ...
        puma_joints_msg.position = self.puma_analytical_IK_calc(pose_msg)

        self.get_logger().info(f'Publishing analytical PUMA joints: {puma_joints_msg.position}')
        self.publisher_.publish(puma_joints_msg)

    def puma_analytical_IK_calc(self, pose_msg):
